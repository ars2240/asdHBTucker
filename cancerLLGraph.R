...
ggp = ggp + geom_point() + scale_x_log10() 
ggp = ggp + theme_bw() + theme(legend.position = "none")
#ggp = ggp + ggtitle("Cancer Log-Likelihood")
ggp = ggp + xlab("Total Topics (log scale)") + ylab("Log-Likelihood")
ggp = ggp + theme(plot.title=element_text(hjust = 0.5))
ggp = ggp + geom_text_repel(aes(label=labels), point.padding=0.1,
                             segment.color='grey50', parse=T)
